...
static bool get_camera_formats_stride_and_size(int w, int h, uint64_t format, int *out_stride, size_t *out_size)
{
	...
	case HAL_PIXEL_FORMAT_RAW12:
		if (w % 2 != 0)
		{
			ALOGE("ERROR: Width for HAL_PIXEL_FORMAT_RAW12 buffers has to be multiple of 2.");
			return false;
		}
	...
	case HAL_PIXEL_FORMAT_BLOB:
		...
		stride = w; /* It's effectively a long one dimensional array */
		size = w;
		break;
		...
		unsigned int ion_flags, uint32_t layer_count, private_handle_t **hnd)
{
	...
	case HAL_PIXEL_FORMAT_RAW10:
	case HAL_PIXEL_FORMAT_BLOB:
		get_camera_formats_stride_and_size(w, h, format, &stride, &size);
...
int mali_gralloc_buffer_allocate(mali_gralloc_module const* m, int format, uint64_t consumer_usage, uint64_t producer_usage, int w, int h, uint32_t layer_count, buffer_handle_t* pHandle)
{
	...
	if (GRALLOC1_SLSI_USAGE_CHECK(producer_usage, GRALLOC1_PRODUCER_USAGE_CAMERA_RESERVED))
	{
		ion_flags |= ION_FLAG_PROTECTED;
	}
#endif
