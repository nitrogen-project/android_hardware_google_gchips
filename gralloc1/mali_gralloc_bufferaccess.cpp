...
int mali_gralloc_lock(const mali_gralloc_module* m, buffer_handle_t buffer, uint64_t usage, int l, int t, int w, int h, void** vaddr)
{
	...
	switch(hnd->format)
	{
		...
		case HAL_PIXEL_FORMAT_EXYNOS_ARGB_8888:
			break;
		default:
			AERR("lock() doesn't support format %x. If using flexible YUV format, try lockYCbCr() instead", hnd->format);
			return -EINVAL;
	}
		...
		struct android_flex_layout* flex_layout, int32_t fence_fd)
{
	...
	{
		AERR("Cannot lock buffer %p with format %" PRIx64 ". No CPU read or write usage flag, or unsupported format", hnd, hnd->internal_format);
		return -EINVAL;
	}
	return 0;
}
